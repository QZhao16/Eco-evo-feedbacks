# Set path
#Root_Path <- 'C:/zhao088/Proposal/Proposal Wageningen UR/Results/Nonlinearity/0.Algae zoo fish whales/PANGAEA/0.(import) microplankton,Sylt Roads,92-13,Wadden Sea/MDR S-map/biomass3'
Root_Path <- 'C:/zhao088/Desk20210304/Manuscript.eco-evo/1.Wadden sea/biomass3'
...
# unit (/Littre)
  new_alg_zo<-read.csv("7.all_01-10.zero.abun.biom.csv")
  new_alg_zoo<-as.data.frame(new_alg_zo)# having year
  dim(new_alg_zoo)
  head(new_alg_zoo)
# scale
...
for (i in 1:dim(alga_zo)[[2]] ) {
  ...
  sim_r <- simplex(alga_zo[,i],lib=c(1,floor(nrow(alga_zo)/2)),pred=c(floor(nrow(alga_zo)/2)+1,nrow(alga_zo)),E=c(2:11))
...
##################################################
##### define delta rho (rho difference)>0.1 over library size   #####
##################################################
# unit (/Littre)
new_alg_zo<-read.csv("7.all_01-10.zero.abun.biom.csv")
new_alg_zoo<-as.data.frame(new_alg_zo)# having year
...
for (i in 1:dim(alga_zo)[[2]]) {
  ...
  for (j in 1:dim(alga_zo)[[2]]) {
    ...
    out.temp <- ccm(block=block_temp,
                    ...
                    lib_column = 1,
                    target_column = 2,
                    lib_sizes = seq(11, nrow(block_temp), by = 10),
                    random_libs = F, 
...
#add date
new_alg<-read.csv("7.all_01-10.zero.abun.biom.csv")
...
for ( l in 1:number_species ) {   #l is end point 
  for ( k in 1:number_species ) { #k is start point
    ...
    #count_all<- sum(count_ae+count_ae2+count_ae3+count_ae4+count_ae5 +count_ae6 )
    count_all<- sum(count_ae+count_ae2)
    
    # merge
    #if (count_all >=1 ) {need_delete<-cbind(l,k) }   
    #if ( count_all >=1 & count_all ==(nrow(su_ae)+nrow(su_ae2)+nrow(su_ae3)+nrow(su_ae4)+nrow(su_ae5) +nrow(su_ae6) ) ) {need_delete<-cbind(l,k) } 
...
###########  find ds ##############
do <- read.csv('7.all_01-10.zero.abun.biom.csv',header=T,stringsAsFactors = F)
...
for ( i in 1:nrow(ds)) {
  ...
  #
  A_matrix<- matrix( Jaco[1:13,1:13], nrow=13, byrow=F)
  B_matrix<- matrix(Jaco[1:13,14], nrow=13, byrow=F)
  C_matrix<- matrix(Jaco[14,1:13], nrow=1, byrow=F)
  D_matrix<- matrix(Jaco[14:14,14:14], nrow=1, byrow=F)
    evo_eco_evo<- C_matrix %*% inv(A_matrix) %*% (-B_matrix)
    eco_evo_eco<- B_matrix %*% (1/D_matrix) %*% (-C_matrix)  
    ...
    #  mx <- which.max(abs(eigs))
      evo_eco_evo_eigen <- evo_eco_evo #abs(eigs)[mx]
    ...
    #  mx <- which.max(abs(eigs))
      D_evo_eco_evo_eigen <- D_evo_eco_evo #abs(eigs)[mx]
  ...
  # D matrix
    ...
    #mx <- which.max(abs(eigs))
    D_matrix_eigen <- D_matrix #abs(eigs)[mx]
...
head(new_alg_zoo)
# scale
alga_zo<-new_alg_zoo[,2:dim(new_alg_zoo)[[2]]] # no year
dim(alga_zo) # 175 6
colnames(alga_zo)
...
#----#----#----#----#----#----#----#----#----
the_tempratures<-read.csv("List_Entrance_Koenigshafen01-10.csv")
the_tempratures<-as.data.frame(the_tempratures)
New_year<-the_tempratures

day_temp<-cbind(as.data.frame(New_year$sampledate), New_year$Temp..�.C., 
                New_year$pH,New_year$O2..µmol.l.,	New_year$Chl.a..µg.l.,	
                New_year$X.PO4.3...µmol.l.,New_year$Si.OH.4..µmol.l.,	New_year$X.NH4....µmol.l.,	
                New_year$X.NO2....µmol.l.,New_year$X.NO3....µmol.l.,	New_year$DON..µmol.l.,	
                New_year$DOP..µmol.l.,	New_year$SPM..mg.l.)
                
                
                
colnames(day_temp)<-c("sampledate","temp_station_all",
                     "pH", "O2",	"Chl.a",
                     "PO4",	"Si.OH.4",	"NH4",
                     "NO2",	"NO3",	"DON",
                     "DOP",	"SPM")
...
for ( i in 1:max(Jocos$time)) { 
  ...
  #evo_eco
  evo_eco_matri<-as.matrix( subs[,1:ncol(subs)] )
  evo_eco_matri[1:(nrow(evo_eco_matri)/2),(ncol(evo_eco_matri)/2+1):ncol(evo_eco_matri)]<-0
  if (i==1){evo.eco_matrix<- diag(ncol(evo_eco_matri))}
  evo.eco_matrix<-evo.eco_matrix %*% evo_eco_matri
  dim(evo.eco_matrix)
  
  #eco_evo
  eco_evo_matri<-as.matrix( subs[,1:ncol(subs)] )
  eco_evo_matri[(ncol(eco_evo_matri)/2+1):ncol(eco_evo_matri),1:(nrow(eco_evo_matri)/2)]<-0
  if (i==1){eco.evo_matrix<- diag(ncol(eco_evo_matri))}
  eco.evo_matrix<-eco.evo_matrix %*% eco_evo_matri
  dim(eco.evo_matrix)
  
  #evo_eco_evo
  evo_eco_evo_matri<-as.matrix( subs[,1:ncol(subs)] )
  evo_eco_evo_matri[1:(nrow(evo_eco_evo_matri)/2),(ncol(evo_eco_evo_matri)/2+1):ncol(evo_eco_evo_matri)]<-0
  evo_eco_evo_matri[(ncol(evo_eco_evo_matri)/2+1):ncol(evo_eco_evo_matri),1:(nrow(evo_eco_evo_matri)/2)]<-0
  if (i==1){evo.eco.evo_matrix<- diag(ncol(evo_eco_evo_matri))}
  evo.eco.evo_matrix<-evo.eco.evo_matrix %*% evo_eco_evo_matri
  dim(evo.eco.evo_matrix)
...
#evo_eco
eigs_evo.eco_matrix <- eigen(evo.eco_matrix)[["values"]]
mx_evo.eco <- which.max(abs(eigs_evo.eco_matrix))
deM_evo.eco <- abs(eigs_evo.eco_matrix)[mx_evo.eco]

#eco_evo
eigs_eco.evo_matrix <- eigen(eco.evo_matrix)[["values"]]
mx_eco.evo <- which.max(abs(eigs_eco.evo_matrix))
deM_eco.evo <- abs(eigs_eco.evo_matrix)[mx_eco.evo]

#evo_eco_evo
eigs_evo.eco.evo_matrix <- eigen(evo.eco.evo_matrix)[["values"]]
mx_evo.eco.evo <- which.max(abs(eigs_evo.eco.evo_matrix))
deM_evo.eco.evo <- abs(eigs_evo.eco.evo_matrix)[mx_evo.eco.evo]
...
#lyapunov exponents
t<-max(Jocos$time)
LE_whole<- log( abs(deM_whole) ) /t
LE_whole
LE_Ecology<-log( abs(deM_Ecology) ) /t 
LE_Ecology
LE_Evolution<-log( abs(deM_Evolution) ) /t  
LE_Evolution
LE_evo.eco<-log( abs(deM_evo.eco) ) /t  
LE_evo.eco
LE_eco.evo<-log( abs(deM_eco.evo) ) /t  
LE_eco.evo
LE_evo.eco.evo<-log( abs(deM_evo.eco.evo) ) /t  
LE_evo.eco.evo
...
#
shan_sim_only_final$LE_evo.eco<-rep(LE_evo.eco,times=nrow(shan_sim_only_final))
shan_sim_only_final$LE_eco.evo<-rep(LE_eco.evo,times=nrow(shan_sim_only_final))
shan_sim_only_final$LE_evo.eco.evo<-rep(LE_evo.eco.evo,times=nrow(shan_sim_only_final))
